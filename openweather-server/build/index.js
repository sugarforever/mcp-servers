...
const isValidCurrentWeatherArgs = (args) => typeof args === 'object' &&
    args !== null &&
...
class OpenWeatherServer {
    ...
    async handleGetCurrentWeather(args) {
        if (!isValidCurrentWeatherArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for get_current_weather');
        }
    ...
    async handleGetWeatherForecast(args) {
        if (!isValidForecastArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for get_weather_forecast');
        }
